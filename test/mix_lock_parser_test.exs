defmodule MixLockParserTest do
  ...
  test "parses mix lock files" do
    dir = Path.relative_to_cwd("priv/sample_lockfiles")
    sample_lockfiles = dir |> File.ls!() |> Enum.sort()

    parsed_lockfiles =
      for file_name <- sample_lockfiles do
        path = Path.join(dir, file_name)
        {path, MixLockParser.read_mix_lock!(path)}
      end

    auto_assert ["dash_mix.lock", "default_phoenix_mix.lock", "exsync_mix.lock", "short_mix.lock"] <-
                  sample_lockfiles

    auto_assert [
                  {"priv/sample_lockfiles/dash_mix.lock",
                   [
                  ...
                  {"priv/sample_lockfiles/default_phoenix_mix.lock",
                   [
                     {:castore, :hex, "1.0.3"},
                     {:cowboy, :hex, "2.10.0"},
                  ...
                  {"priv/sample_lockfiles/exsync_mix.lock",
                   [
                     {:earmark, :hex, "1.3.2"},
                     {:ex_doc, :hex, "0.21.1"},
                  ...
                  {"priv/sample_lockfiles/short_mix.lock", [{:nimble_parsec, :hex, "1.3.1"}]}
                ] <- parsed_lockfiles
  end
end
